...
class SoxHandler(BaseHandler):
    def __init__(self, config, callback_report):
        ...
        self.parametersSpeech = []
        self.parametersMedia = []

        self.proc = None

    def on_setup(self):
        if self.__config['sound']['output']:
            self.parametersSpeech.extend(['-t', self.__config['sound']['output']])
            if self.__config['sound']['output_device']:
                self.parametersSpeech.append(self.__config['sound']['output_device'])

        if self.__config['sound']['media_output']:
            self.parametersMedia.extend(['-t', self.__config['sound']['media_output']])
            if self.__config['sound']['media_output_device']:
                self.parametersMedia.append(self.__config['sound']['media_output_device'])
        else:
            self.parametersMedia = self.parametersSpeech
    ...
    def on_play(self, item):
        ...
        if item.audio_type == PlaybackAudioType.SPEECH:
            sox_cmd.extend(self.parametersSpeech)
            sox_cmd.extend(['vol', str(self.volume_gain), 'dB'])
        else:
            sox_cmd.extend(self.parametersMedia)
